...
Recursive function have two main parts: base case and recursive case. Base case is basically a stop-sign for a recursive function and it is binded to certain conditions. Recursive case is part of function body where the function calls itself. Each recursive call must return some value. Recursive calls are handled via call stack [1]. Tail recursion is special case of recursion when a recursive part of a function ends with a recursion, instead of an arithmetic expression (recursion at tail = tail recursion :D). Also, there are direct and indirect recursion types. Direct recursion happens when a function calls itself, indirect recursion happens when two function call each other [2].
